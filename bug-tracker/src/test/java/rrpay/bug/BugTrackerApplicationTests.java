...
class BugTrackerApplicationTests {
	...
	void shouldCreateBug() {
		String requestBody = """
				...
				""";
		RestAssured.given().contentType("application/json").body(requestBody).when().post("/bugs/create").then().log().all().statusCode(201).body("id", Matchers.notNullValue()).body("code", Matchers.equalTo("INSTALMENTPAY-001")).body("summary", Matchers.equalTo("first bug in payment in instalments")).body("description", Matchers.equalTo("bug desc")).body("priority", Matchers.is("LOW"));
	}
/*
	@Test
	public void getBugs() {
